...
class GameCollectionVC: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        let newLabelBuilder = LabelBuilder(textColor: UIColor(displayP3Red: 150/255, green: 10/255, blue: 10/255, alpha: 1), fontName: "SFCompactDisplay-Regular")
        newLabelBuilder.setText(text: "Hit!")
        newLabelBuilder.setFontSize(fontSize: 35)
        newLabelBuilder.setFrame(frame: CGRect(x: 282, y: 492, width: 204, height: 44))
        self.indicationLabel = newLabelBuilder.returnLabel()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let labelBuilder = LabelBuilder(textColor: UIColor(displayP3Red: 248/255, green: 240/255, blue: 240/255, alpha: 1), fontName: "SFCompactDisplay-Regular")
        labelBuilder.setText(text: gameState.transitionSentence)
        labelBuilder.setFontSize(fontSize: 35)
        labelBuilder.setFrame(frame: CGRect(x: 195, y: 260, width: 768, height: 57))
        self.transitionLabel = labelBuilder.returnLabel()
        self.transitionLabel.center = CGPoint(x: self.view.bounds.width/2, y: self.view.bounds.height/2)
